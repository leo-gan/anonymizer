...
from id_extract.plugins import (
    COUNTRY_PATTERN_SUFFIXES as COUNTRY_PATTERN_SUFFIXES,
    filter_regex_patterns,
    pattern_country as pattern_country,
    patterns as _id_patterns,
)
...
# First-stage regex patterns live in the id-extract package. This name is
# kept so existing callers and tests do not change. Default is every
# bundled country plus the universal keys (EMAIL, IBAN, cards, ...).
DEFAULT_REGEX_PATTERNS: Dict[str, str] = _id_patterns(countries="all")
