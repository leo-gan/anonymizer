...
| `DEA_US` | DEA registration number | Federal | 2 letters and 7 digits. Under [21 CFR 1301.22(c)](https://www.ecfr.gov/current/title-21/chapter-II/part-1301/section-1301.22) a hospital may add a hyphen and an internal suffix. | Strong | `AB1234567` | [DEA Practitioner's Manual](<https://www.deadiversion.usdoj.gov/GDP/(DEA-DC-071)(EO-DEA226)_Practitioner's_Manual_(final).pdf>) |
...
`ITIN_US` and `ATIN_US` use the same hyphens as `SSN_US`. Test those two patterns first, or every ITIN is labeled a Social Security number. `DEA_US` sits inside `MEDICAL_LICENSE_US`. `DL_US_FL` sits inside `DRIVERS_LICENSE_US`. A hyphenated NDC can also be ten digits and sit inside `MEDICAL_NPI_US`, which is any ten digits. The tighter pattern should win the overlap.
...
| `NAM_CA_QC` | Quebec health-insurance number (NAM) | Province | 4 letters and 8 digits | Strong | `ABCD12345678` | [RAMQ Infolettre 011](https://www.ramq.gouv.qc.ca/SiteCollectionDocuments/professionnels/infolettres/2015/info011-5.pdf) |
...
| `DL_CA_ON` | Ontario driver-licence number | Province | 1 letter and 14 digits. The last two digits are `01`–`31`. | Strong | `A12345678901231` | [Ontario design-system hint](https://designsystem.ontario.ca/components/preview/hint-text.html) |
...
`DL_CA_ON` sits inside the current `DRIVERS_LICENSE_CA` pattern. The Ontario pattern should win that overlap. `NAM_CA_QC` does not match `SIN_CA`, because the SIN pattern requires hyphens.
